...
def main():
    ...
    try: 
        ...
        stats_df['gp'] = stats_df['gp'].astype(int)
        ...
        stats_df['recieve_avg'] = stats_df['recieve_avg'].fillna(0).astype(int).round(1)
        ...
        with open('src/queries/stats_insert_2023.sql', 'r') as query:
            sql = query.read()
        for index, row in stats_df.iterrows():
